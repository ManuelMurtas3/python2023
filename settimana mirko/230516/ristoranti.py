...
def switch_menu():
    ...
    while option != "3":
        ...
        if option == "1":
            # ordinazione piatto
            piatto = switch_piatti("Ordina il tuo piatto")
            ordinazione.ordina_piatto(piatto)

        elif option == "2":
            # modifica piatto
            piatto = switch_piatti("Seleziona che piatto modificare")
            nome = input("Inserisci il nuovo nome del piatto: ")
            prezzo = float(input("Inserisci il nuovo prezzo del piatto: € "))
            piatto.modifica_nome(nome)
            piatto.modifica_prezzo(prezzo)
